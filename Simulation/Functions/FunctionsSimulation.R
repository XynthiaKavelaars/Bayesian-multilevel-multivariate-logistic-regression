...
EvaluateData <- function(Data, Weights, Alpha, Alternative = "greater.than", Rule, Truth = NULL, Types){
  ...
  nIt <- length(Data)
  #DeltaEst <- lapply(1:nIt, function(i) do.call(rbind, lapply(1:length(Data), function(j) Data[[j]][[i]])))
  DeltaEst <- Data
